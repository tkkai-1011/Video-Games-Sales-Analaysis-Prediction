...
val VideoGamesSales = spark.read.format("csv")
  ...
  .load("/Users/anhdao/Desktop/CSYE7200/VideoGamesSales/Video-Games-Sales-Analaysis-Prediction/src/main/Edited3_Video_Games_Sales_as_at_22_Dec_2016.csv")
...
//  val RatingEncoder = new OneHotEncoder().setInputCol("RatingIndex").setOutputCol("RatingVec")
  ...
  val features = VideoGamesSales.columns.filterNot(_.contains("Sales")).filterNot(_.contains("r_Score"))
  ...
  val indexer_model = pipeline.fit(VideoGamesSales)

  val df_transformed = indexer_model.transform(VideoGamesSales)
